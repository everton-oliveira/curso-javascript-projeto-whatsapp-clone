...
export class Message extends Model {
    ...
    static send(chatId, from, type, content) {
        ...
        return new Promise((s, f) => {
            Message.getRef(chatId).add({
                ...
                Message.updateStatusSentMessage(result);
            });
    ...
    static sendPDFDocument(chatId, from, file, filePreview, info) {

        Message.upload(file, from).then(snapshot => {
            snapshot.ref.getDownloadURL().then(downloadURL => {

                Message.upload(filePreview, from).then(snapshotPreview => {
                    snapshotPreview.ref.getDownloadURL().then(downloadURLPreview => {
    ...
    static sendDocument(chatId, from, file) {
    ...
    static updateStatusSentMessage(message) {
        message.parent.doc(message.id).set({
            status: 'sent'
        }, {
            merge: true
        });        
